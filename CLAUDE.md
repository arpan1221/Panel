...
3. **Cross-experiment knowledge base is pitched, not fully demoed.** Ship the scaffolding (Archivist writes entries, later experiments retrieve them). Do not attempt to prove cumulative learning with experiments over time — there isn't time.
...
/examples     Pre-baked experiments used for demo: titanic, house_prices
```
...
## The two canonical test experiments
...
If a feature works on Titanic but not on House Prices, it's not done.
...
- **Saturday:** share page + polish + demo video
