...
# Allow-listed canonical datasets. Keep this small and explicit.
...
# Map an experiment's recorded dataset path back to a canonical name so the
# Colab-portable notebook can wget it from /datasets/{name}.
...
def make_colab_portable(notebook: dict, *, share_url: str, dataset_path: str | None,
    ...
    """Prepend intro + setup cells so the notebook runs in Colab unchanged."""
...
def get_dataset(name: str) -> FileResponse:
    """Serve a canonical example dataset. Allow-list only."""
...
def get_notebook(experiment_id: str, request: Request, colab: int = 0) -> JSONResponse:
    rd = run_dir_for(experiment_id)
    ...
    if colab:
        ...
        # Build URLs from the request so they resolve from wherever the user is browsing.
        base = str(request.base_url).rstrip("/")
        share_url = f"{base.replace('/api', '')}/share/{experiment_id}"
        # The web UI lives on a different port in dev; if the caller sent a
        # Referer we can do better, but don't bother for v0 — the URL is just
        # decorative inside the notebook.
        dataset_url = f"{base}/datasets/{canonical}" if canonical else None
